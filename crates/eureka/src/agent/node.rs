...
//!
//! Node-level parallelism is provided by replica expansion in the scheduler
//! (controlled by `[agent] workers` in the runtime TOML config), not by
//! internal LLM call fan-out.
    ...
    async fn process(
        ...
        let (output_json, usage) = self
            ...
            .run_agent_loop_with_environment(
                ...
                ctx.event_tx.clone(),
                &self.environment,
                ctx.live_tokens.clone(),
                ctx.live_input_tokens.clone(),
                ctx.live_output_tokens.clone(),
                ctx.live_cost.clone(),
            )
            .await
