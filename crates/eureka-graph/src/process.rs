//! Shared subprocess runner used by both agent tools and plugin control nodes.
//!
//! Both [`CommandTool`](crate::boxed::CommandTool) and
//! [`ControlPluginNode`](crate::boxed::ControlPluginNode) follow the same
...
/// # Errors
///
/// Returns a [`ProcessError`] if the process cannot be spawned, communication
...
pub async fn run_subprocess(
    ...
    let output = match tokio::time::timeout(timeout, child.wait_with_output()).await {
        ...
        Err(_elapsed) => {
            return Err(ProcessError::Timeout { timeout_secs });
        }
    };
